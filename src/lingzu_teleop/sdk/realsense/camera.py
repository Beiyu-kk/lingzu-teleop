"""Intel RealSense RGB-D camera helpers.
...
logger = logging.getLogger("lingzu_teleop.sdk.realsense.camera")
...
class RealSenseD435:
    ...
    FALLBACK_RGBD_MODES: Sequence[Tuple[int, int, int, int]] = (
        (1280, 720, 1280, 720),
        (848, 480, 848, 480),
        (640, 480, 640, 480),
        (640, 480, 640, 360),
    ...
    def start(self) -> None:
        ...
        try:
            ...
            message = str(exc)
            if "Couldn't resolve requests" in message and self.auto_fallback:
                self._profile = self._start_with_fallback(exc)
            elif "Couldn't resolve requests" in message:
                raise RuntimeError(
                ...
                raise
        self._update_active_stream_settings()
        depth_sensor = self._profile.get_device().first_depth_sensor()
        self._depth_scale = float(depth_sensor.get_depth_scale())

    def _start_with_fallback(self, first_error: Exception):
        requested = (self.width, self.height, self.depth_width, self.depth_height)
        tried = [requested]
        for width, height, depth_width, depth_height in self.FALLBACK_RGBD_MODES:
            candidate = (width, height, depth_width, depth_height)
            ...
            try:
                ...
                if "Couldn't resolve requests" not in str(exc):
                    raise
                continue
            self._config = config
            ...
            logger.warning(
                "RealSense requested RGB-D mode is unsupported; using "
                "color=%sx%s@%s depth=%sx%s@%s",
                self.width,
                self.height,
                self.fps,
                self.depth_width,
        ...
        tried_text = ", ".join(
            f"color={w}x{h}@{self.fps}, depth={dw}x{dh}@{self.fps}"
            for w, h, dw, dh in tried
        )
        raise RuntimeError(
            "RealSense 不支持当前 RGB-D 流配置，自动降级也未找到可用模式。"
            f"已尝试：{tried_text}。请在 RealSense Viewer 中确认相机支持的深度和彩色分辨率。"
        ) from first_error

    def _update_active_stream_settings(self) -> None:
        ...
        for stream_profile in self._profile.get_streams():
            try:
                ...
                continue
            if stream_type == self._rs.stream.color:
                self.width = width
                self.height = height
            elif stream_type == self._rs.stream.depth:
